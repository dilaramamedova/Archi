...
<x-layout page="business-inventory" :title="__('business-inventory.title')" bodyClass="bg-gray-soft2">
...
<x-cabinet.shell ns="business-inventory" active="inventory" class="text-ink"
    :heading="__('business-inventory.heading')" :nav-items="$businessNav" :show-view-button="false">
  ...
  <div class="grid w-full grid-cols-3 gap-4 max-[700px]:grid-cols-1">
    <div class="cab-card flex flex-col gap-1.5 p-5">
      ...
      <p class="text-[13px] text-black/50">{{ __('business-inventory.kpi.total') }}</p>
    </div>
    <div class="cab-card flex flex-col gap-1.5 p-5">
      <p class="text-[26px] font-bold text-[#b86114]">{{ number_format($stats['low']) }}</p>
      <p class="text-[13px] text-black/50">{{ __('business-inventory.kpi.low') }}</p>
    </div>
    <div class="cab-card flex flex-col gap-1.5 p-5">
      <p class="text-[26px] font-bold text-[#e5484d]">{{ number_format($stats['out']) }}</p>
      <p class="text-[13px] text-black/50">{{ __('business-inventory.kpi.out') }}</p>
    </div>
  </div>
    ...
    <div class="flex flex-wrap gap-2">
      ...
      @foreach ([
        ...
        <a href="{{ route('business.inventory', array_filter(['filter' => $fk === 'all' ? null : $fk, 'q' => request('q')])) }}"
          ...
          {{ __('business-inventory.filters.' . $fk) }} · {{ number_format($fcount) }}
        </a>
      @endforeach
  ...
  <div class="cab-card overflow-hidden p-0">
    @if ($products->isEmpty())
      <x-ui.empty-state icon="📦" :title="__('business-inventory.empty.title')" :description="__('business-inventory.empty.desc')" />
    @else
      <div class="w-full overflow-x-auto">
        <table class="w-full min-w-[760px]">
          <thead>
            <tr class="bg-gray-soft2">
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-black/50">{{ __('business-inventory.table.product') }}</th>
              <th class="w-[90px] px-3 py-3.5 text-left text-xs font-semibold text-black/50">{{ __('business-inventory.table.shelf') }}</th>
              <th class="w-[110px] px-3 py-3.5 text-left text-xs font-semibold text-black/50">{{ __('business-inventory.table.stock') }}</th>
              <th class="w-[130px] px-3 py-3.5 text-left text-xs font-semibold text-black/50">{{ __('business-inventory.table.status') }}</th>
          ...
          <tbody>
            @foreach ($products as $product)
              @php
                ...
                $unitLabel = __('business-product-edit.pricing.units')[$product->unit] ?? '';
              @endphp
              <tr class="border-t border-black/8" data-inventory-row data-product-id="{{ $product->id }}">
                ...
                <td class="px-3 py-3.5">
                  @if ($stockState === 'ok')
                    <span class="rounded bg-[#e9f6ed] px-3 py-1.5 text-[13px] font-semibold text-[#229653]">{{ __('business-inventory.table.status_ok') }}</span>
                  @elseif ($stockState === 'low')
                    <span class="rounded bg-[#fff3e0] px-3 py-1.5 text-[13px] font-semibold text-[#b86114]">{{ __('business-inventory.table.status_low') }}</span>
                  @else
                    <span class="rounded bg-[#fdecec] px-3 py-1.5 text-[13px] font-semibold text-[#e5484d]">{{ __('business-inventory.table.status_out') }}</span>
                  @endif
                </td>
                <td class="px-5 py-3.5 text-right">
                  <button type="button" data-add-stock data-product-id="{{ $product->id }}" data-current="{{ $product->stock }}"
                    ...
                    {{ __('business-inventory.table.add_stock') }}
                  </button>
                </td>
              </tr>
            @endforeach
  ...
  <div id="stockModal" class="fixed inset-0 z-[9998] hidden items-center justify-center bg-black/40">
    <div class="mx-4 w-full max-w-[380px] rounded border border-black/10 bg-white p-7 shadow-xl">
      <h3 class="text-lg font-bold text-ink">{{ __('business-inventory.table.add_stock_title') }}</h3>
      <label class="mt-4 block text-[13px] font-semibold text-black/90" for="stockInput">{{ __('business-inventory.table.add_stock_label') }}</label>
      ...
      <div class="mt-5 flex justify-end gap-2.5">
        <button type="button" id="stockCancel"
                class="rounded border border-black/10 px-5 py-2.5 text-sm font-semibold text-ink transition hover:bg-gray-soft2">{{ __('common.cancel') }}</button>
        <button type="button" id="stockSave"
                class="rounded bg-yellow px-5 py-2.5 text-sm font-semibold text-ink transition hover:brightness-95">{{ __('common.save') }}</button>
      </div>
    </div>
  </div>
