<x-layout page="business-onboarding-step1" :title="t('business-onboarding-step1.title')" bodyClass="bg-gray-soft2 [word-break:break-word]">
...
<div class="mx-auto flex max-w-[1440px] flex-col items-start gap-5 px-7 pt-8 pb-5">
  <a class="shrink-0 whitespace-pre text-sm font-medium leading-[normal] text-black/50" href="{{ route('business.profile') }}">{{ t('business-onboarding-step1.back') }}</a>
  <h1 class="shrink-0 whitespace-nowrap text-[32px] font-bold leading-[normal] tracking-[-0.5px] text-ink">{{ t('business-onboarding-step1.heading') }}</h1>
</div>
...
<div class="mx-auto flex max-w-[1440px] items-start gap-8 px-7 pb-5 max-[1200px]:flex-col">
  <div class="flex w-full max-w-[880px] flex-col items-start gap-2.5 max-[1200px]:max-w-none">
    ...
    <div class="flex w-full items-center justify-between border border-black/10 bg-white p-5 max-[640px]:flex-col max-[640px]:items-start max-[640px]:gap-3">
      <div class="flex shrink-0 items-center gap-2">
        ...
        <p class="whitespace-nowrap text-sm font-bold leading-[normal] text-ink">{{ t('business-onboarding-step1.steps.company') }}</p>
      </div>
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        <p class="whitespace-nowrap text-sm font-medium leading-[normal] text-[#808085]">{{ t('business-onboarding-step1.steps.contact') }}</p>
      </div>
      <div class="h-0.5 w-5 shrink-0 bg-[#e0e3e8]"></div>
      <div class="flex shrink-0 items-center gap-2">
        <div class="flex size-7 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#e5e8ed]">
        ...
        <p class="whitespace-nowrap text-sm font-medium leading-[normal] text-[#808085]">{{ t('business-onboarding-step1.steps.product') }}</p>
      </div>
    </div>
    ...
    <div class="flex w-full flex-col items-start gap-5 rounded-ds border border-black/10 bg-white px-8 py-7">
      <div class="flex w-full items-start gap-4 overflow-hidden bg-white max-[640px]:flex-col">
        <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.legal_name_label')" for="onb1-legal-name"
          ...
          <x-ui.input variant="b2b" id="onb1-legal-name" name="legal_name" autocomplete="organization"
                      :placeholder="t('business-onboarding-step1.form.legal_name_placeholder')" />
        </x-ui.field>
        <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.brand_label')" for="onb1-brand"
                    class="relative min-w-px flex-[1_0_0] items-start overflow-hidden bg-white max-[640px]:w-full">
          <x-ui.input variant="b2b" id="onb1-brand" name="brand"
                      :placeholder="t('business-onboarding-step1.form.brand_placeholder')" />
        </x-ui.field>
      </div>

      <div class="flex w-full items-start gap-0 overflow-hidden bg-white">
        <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.tax_id_label')" for="onb1-tax-id"
          ...
          <x-ui.input variant="b2b" id="onb1-tax-id" name="tax_id" inputmode="numeric" maxlength="10"
                      :placeholder="t('business-onboarding-step1.form.tax_id_placeholder')" />
        </x-ui.field>
      </div>
      ...
      <div class="flex w-full items-start gap-4 bg-white max-[640px]:flex-col">
        ...
        <x-ui.field variant="b2b" class="relative min-w-px flex-[1_0_0] items-start bg-white max-[640px]:w-full">
          <p class="ui-label-b2b" id="onb1-city-label">{{ t('business-onboarding-step1.form.city_label') }}</p>
          ...
          <div class="flex w-full shrink-0 cursor-pointer items-center justify-between overflow-hidden rounded-ds border-[1.5px] border-black/14 bg-white px-[15.5px] py-[12.5px] data-[on=true]:border-ink"
            ...
            <p class="whitespace-nowrap text-sm font-normal leading-[normal] text-black/40 data-[filled=true]:text-ink" data-city-value>{{ t('business-onboarding-step1.form.city_placeholder') }}</p>
          ...
          <div class="absolute top-[calc(100%+4px)] left-0 z-20 flex max-h-[300px] w-full flex-col items-start overflow-y-auto rounded-ds border border-black/10 bg-white shadow-[0px_4px_16px_0px_rgba(0,0,0,0.08)] data-[on=false]:hidden"
            ...
            @foreach (t('business-onboarding-step1.cities') as $city)
        ...
        <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.address_label')" for="onb1-address"
                    class="relative min-w-px flex-[1_0_0] items-start overflow-hidden bg-white max-[640px]:w-full">
          <x-ui.input variant="b2b" id="onb1-address" name="address" autocomplete="street-address"
                      :placeholder="t('business-onboarding-step1.form.address_placeholder')" />
        </x-ui.field>
      </div>
      ...
      <div class="flex w-full items-start gap-4 overflow-hidden bg-white max-[640px]:flex-col">
        <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.phone_label')" for="onb1-phone"
          ...
          <x-ui.input variant="b2b" id="onb1-phone" type="tel" name="phone" autocomplete="tel"
                      ...
                      :placeholder="t('business-onboarding-step1.form.phone_placeholder')" />
        </x-ui.field>
        <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.showroom_label')" for="onb1-showroom"
                    class="relative min-w-px flex-[1_0_0] items-start gap-1.5 max-[640px]:w-full">
          <x-ui.input variant="b2b" id="onb1-showroom" name="showroom"
                      class="border-black/10 px-3.5 py-3 placeholder:text-[#808085]"
                      :placeholder="t('business-onboarding-step1.form.showroom_placeholder')" />
        </x-ui.field>
      </div>
      ...
      <div class="flex w-full flex-none items-start">
        <x-ui.field variant="b2b" tag="p" :label="t('business-onboarding-step1.form.logo_label')"
          ...
          <label class="flex w-full shrink-0 cursor-pointer items-center justify-center border border-dashed border-black/25 p-[22px]" for="onb1-logo">
            <input class="sr-only" id="onb1-logo" type="file" name="logo" accept=".png,.svg,image/png,image/svg+xml" data-logo-input data-empty-label="{{ t('business-onboarding-step1.form.logo_upload') }}">
            <p class="min-w-0 max-w-full overflow-hidden text-ellipsis whitespace-pre text-sm font-semibold leading-[normal] text-[#595966]" data-logo-text>{{ t('business-onboarding-step1.form.logo_upload') }}</p>
          </label>
        </x-ui.field>
      </div>

      <x-ui.field variant="b2b" :label="t('business-onboarding-step1.form.about_label')" for="onb1-about"
        ...
        <x-ui.textarea variant="b2b" id="onb1-about" name="about" class="h-24 resize-none"
                       :placeholder="t('business-onboarding-step1.form.about_placeholder')" />
      </x-ui.field>
    </div>
    ...
    <div class="flex w-full shrink-0 flex-col items-start overflow-hidden rounded-ds border border-black/10 bg-white p-6">
      <div class="flex w-full items-center gap-3 overflow-hidden bg-white">
        ...
        <x-ui.button variant="primary" :hover="false" :href="route('business.onboarding.step2')" class="px-7 py-[15px]">
          <p class="shrink-0 whitespace-pre text-[15px] font-semibold leading-[normal] text-ink">{{ t('business-onboarding-step1.actions.save') }}</p>
        </x-ui.button>
        <a class="shrink-0 whitespace-nowrap text-sm font-medium leading-[normal] text-black/50" href="{{ route('business.profile') }}">{{ t('business-onboarding-step1.actions.later') }}</a>
      </div>
    </div>
  ...
  <div class="relative flex min-w-px flex-[1_0_0] flex-col items-start gap-4 overflow-hidden bg-white max-[1200px]:w-full max-[1200px]:flex-none">
    <div class="flex w-full shrink-0 flex-col items-start gap-3 overflow-hidden rounded-ds border border-black/10 bg-white p-5">
      <p class="shrink-0 whitespace-nowrap text-sm font-semibold leading-[normal] text-ink">{{ t('business-onboarding-step1.side.progress_title') }}</p>
      ...
      <p class="w-full shrink-0 text-[13px] font-normal leading-[normal] text-black/50">{{ t('business-onboarding-step1.side.progress_note') }}</p>
    </div>
    <div class="flex w-full shrink-0 flex-col items-start gap-2 overflow-hidden rounded-ds border border-yellow-line bg-sel-bg p-4 text-[13px]">
      <p class="shrink-0 whitespace-nowrap font-semibold leading-[normal] text-ink">{{ t('business-onboarding-step1.side.tip_title') }}</p>
      <p class="w-full shrink-0 font-normal leading-[1.5] text-black/70">{{ t('business-onboarding-step1.side.tip_text') }}</p>
    </div>
  </div>
