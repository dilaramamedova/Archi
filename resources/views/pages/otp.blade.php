<x-layout page="otp" :title="t('auth.otp_title')">
  ...
  <div class="mx-auto max-w-[640px] px-7">
    <div class="flex flex-col gap-6 rounded border border-black/10 bg-white px-12 py-10 max-[640px]:px-6 max-[640px]:py-8">
      <div class="flex flex-col gap-2">
        <x-ui.eyebrow :label="t('auth.otp_tag')" />
        <h1 class="text-[32px] font-bold tracking-[-0.5px] text-black/90 max-[640px]:text-[26px]">{{ t('auth.otp_heading') }}</h1>
      ...
      <form class="flex flex-col gap-6" id="otpForm">
        ...
        <x-ui.button variant="primary" type="submit"
                     class="h-[54px] w-full rounded text-base font-semibold">{{ t('auth.otp_submit') }}</x-ui.button>
        <p class="text-center text-[13px] text-black/50" id="otpTimer">{{ t('auth.otp_no_code') }} <span class="font-semibold text-ink" id="otpCountdown">{{ t('auth.otp_resend_wait', ['seconds' => 60]) }}</span></p>
      </form>
    </div>
  </div>
</section>
