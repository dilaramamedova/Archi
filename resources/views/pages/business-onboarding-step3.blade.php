<x-layout page="business-onboarding-step3" :title="t('business-onboarding-step3.title')" bodyClass="bg-gray-soft2 text-ink [word-break:break-word]">
...
<div class="mx-auto flex max-w-[1440px] flex-col items-start gap-5 px-7 pt-8 pb-5">
  <a class="text-sm font-medium leading-[normal] whitespace-pre text-black/50" href="{{ route('business.profile') }}">{{ t('business-onboarding-step3.head.back') }}</a>
  <h1 class="text-[32px] font-bold tracking-[-0.5px] leading-[normal] whitespace-nowrap text-ink">{{ t('business-onboarding-step3.head.title') }}</h1>
</div>
...
<div class="mx-auto flex max-w-[1440px] items-start gap-8 px-7 pb-5">
  <div class="flex w-[880px] shrink-0 flex-col items-start gap-2.5">
    ...
    <div class="flex w-[880px] shrink-0 items-center justify-between border border-black/10 bg-white p-5">
      <div class="flex shrink-0 items-center gap-2">
        ...
        <p class="text-sm font-medium leading-[normal] whitespace-nowrap text-ink">{{ t('business-onboarding-step3.stepper.step1') }}</p>
      </div>
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        <p class="text-sm font-medium leading-[normal] whitespace-nowrap text-ink">{{ t('business-onboarding-step3.stepper.step2') }}</p>
      </div>
      <div class="h-0.5 w-5 shrink-0 bg-[#e0e3e8]"></div>
      <div class="flex shrink-0 items-center gap-2.5">
        ...
        <p class="text-[13px] font-medium leading-[normal] whitespace-nowrap text-ink">{{ t('business-onboarding-step3.stepper.step3') }}</p>
      </div>
    </div>
    ...
    <x-ui.card class="flex shrink-0 flex-col items-start gap-5 px-8 py-7">
      ...
      <x-ui.field variant="b2b" tag="p" :label="t('business-onboarding-step3.form.images_label')"
        ...
        <div class="flex shrink-0 items-start gap-2.5 overflow-hidden bg-white">
          <div class="flex size-[110px] shrink-0 cursor-pointer flex-col items-center justify-center overflow-hidden rounded-ds border border-dashed border-black/14 bg-gray-soft2 text-center text-xs font-medium text-black/50">
            ...
            <p class="leading-[normal] whitespace-nowrap">{{ t('business-onboarding-step3.form.images_main') }}</p>
          </div>
      ...
      <div class="flex w-full shrink-0 items-start gap-4 bg-white">
        <x-ui.field variant="b2b" :label="t('business-onboarding-step3.form.name_label')"
                    class="min-w-px flex-[1_0_0] items-start overflow-hidden bg-white">
          <x-ui.input variant="b2b" :placeholder="t('business-onboarding-step3.form.name_placeholder')" />
        </x-ui.field>

        <x-ui.field variant="b2b" :label="t('business-onboarding-step3.form.category_label')"
          ...
          <div class="flex w-full shrink-0 cursor-pointer items-center justify-between overflow-hidden rounded-ds border-[1.5px] border-black/14 bg-white px-[15.5px] py-[12.5px] data-[on=true]:border-ink" data-cat-trigger data-on="false">
            <p class="text-sm font-normal leading-[normal] whitespace-nowrap text-black/40 data-[filled=true]:text-ink" data-cat-value>{{ t('business-onboarding-step3.form.category_placeholder') }}</p>
          ...
          <div class="absolute top-[calc(100%+4px)] left-0 z-20 flex max-h-[300px] w-full flex-col items-start overflow-y-auto rounded-ds border border-black/10 bg-white shadow-[0px_4px_16px_0px_rgba(0,0,0,0.08)] data-[on=false]:hidden" data-cat-menu data-on="false">
            @foreach (t('business-onboarding-step3.categories') as $category)
      ...
      <div class="flex w-full shrink-0 items-start gap-0 overflow-hidden bg-white">
        <x-ui.field variant="b2b" :label="t('business-onboarding-step3.form.brand_label')"
                    class="min-w-px flex-[1_0_0] items-start overflow-hidden bg-white">
          <x-ui.input variant="b2b" :placeholder="t('business-onboarding-step3.form.brand_placeholder')" />
        </x-ui.field>
      </div>

      <div class="flex w-full shrink-0 items-start gap-4 overflow-hidden bg-white">
        <x-ui.field variant="b2b" :label="t('business-onboarding-step3.form.price_label')"
                    class="min-w-px flex-[1_0_0] items-start overflow-hidden bg-white">
          <x-ui.input variant="b2b" :placeholder="t('business-onboarding-step3.form.price_placeholder')" />
        </x-ui.field>

        {{-- Unit picker: a static box, not a <select> — the control stays page-local. --}}
        <x-ui.field variant="b2b" :label="t('business-onboarding-step3.form.unit_label')"
                    class="min-w-px flex-[1_0_0] items-start overflow-hidden bg-white">
          <div class="flex w-full shrink-0 items-center justify-between overflow-hidden rounded-ds border border-black/14 bg-white px-4 py-[13px]">
            <p class="text-sm font-normal leading-[normal] whitespace-nowrap text-black/40">{{ t('business-onboarding-step3.form.unit_placeholder') }}</p>
            <p class="text-xs leading-[normal] text-black/50">▾</p>
          </div>
        </x-ui.field>
      </div>

      <x-ui.field variant="b2b" :label="t('business-onboarding-step3.form.desc_label')"
                  class="w-full shrink-0 items-start overflow-hidden bg-white">
        <x-ui.input variant="b2b" :placeholder="t('business-onboarding-step3.form.desc_placeholder')" />
      </x-ui.field>
      ...
      <div class="flex w-full shrink-0 items-center gap-2.5 overflow-hidden rounded-ds border border-yellow-line bg-sel-bg px-3.5 py-3">
        ...
        <p class="min-w-px flex-[1_0_0] text-[13px] font-normal leading-[normal] text-black/70">{{ t('business-onboarding-step3.form.note') }}</p>
      </div>
      ...
      <div class="flex shrink-0 items-center gap-3 overflow-hidden bg-white">
        ...
        <x-ui.button variant="primary" :hover="false" :href="route('business.profile')" class="items-start px-7 py-[15px]">
          <p class="font-sans text-[15px] font-semibold leading-[normal] whitespace-pre text-ink">{{ t('business-onboarding-step3.form.submit') }}</p>
        </x-ui.button>
        <p class="cursor-pointer text-sm font-medium leading-[normal] whitespace-nowrap text-black/50">{{ t('business-onboarding-step3.form.later') }}</p>
      </div>
    </x-ui.card>
  </div>
  ...
  <div class="flex min-w-px flex-[1_0_0] flex-col items-start gap-4 overflow-hidden bg-white">
    <x-ui.card class="flex shrink-0 flex-col items-start gap-3 overflow-hidden p-5">
      <p class="text-sm font-semibold leading-[normal] whitespace-nowrap text-ink">{{ t('business-onboarding-step3.side.progress_title') }}</p>
      ...
      <p class="w-full text-[13px] font-normal leading-[normal] text-black/50">{{ t('business-onboarding-step3.side.progress_note') }}</p>
    </x-ui.card>
    <div class="flex w-full shrink-0 flex-col items-start gap-2 overflow-hidden rounded-ds border border-yellow-line bg-sel-bg p-4 text-[13px]">
      <p class="font-semibold leading-[normal] whitespace-nowrap text-ink">{{ t('business-onboarding-step3.side.tip_title') }}</p>
      <p class="w-full leading-[1.5] font-normal text-black/70">{{ t('business-onboarding-step3.side.tip_text') }}</p>
    </div>
  </div>
</div>
