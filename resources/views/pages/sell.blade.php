...
<section class="bg-gray-soft2 py-12">
  <div class="mx-auto max-w-[1040px] px-7">
    <div class="mb-7 flex flex-col gap-2">
      <x-ui.eyebrow :label="__('sell.hero.tag')" />
      ...
      <div class="grid grid-cols-[360px_1fr] gap-8 max-[860px]:grid-cols-1">
        ...
        <div class="flex flex-col gap-[18px]">
          <x-ui.field class="flex-1">
            ...
            <label class="ui-label" for="pName">{{ __('sell.form.name_label') }} <span class="text-red">*</span></label>
            <x-ui.input :class="$control" id="pName" :placeholder="__('sell.form.name_placeholder')" required />
          </x-ui.field>
          ...
          <div class="flex gap-3.5 max-[560px]:flex-col">
            <x-ui.field class="flex-1">
              <label class="ui-label" for="pPrice">{{ __('sell.form.price_label') }} <span class="text-red">*</span></label>
              <div class="relative flex items-center"><x-ui.input type="number" :class="$control . ' flex-1 pr-[38px]'" id="pPrice" placeholder="0" min="0" step="0.01" /><span class="pointer-events-none absolute right-3.5 text-[15px] text-black/45">{{ __('sell.form.currency') }}</span></div>
            </x-ui.field>
            <x-ui.field class="flex-1" :label="__('sell.form.old_price_label')" for="pOld">
              <div class="relative flex items-center"><x-ui.input type="number" :class="$control . ' flex-1 pr-[38px]'" id="pOld" placeholder="0" min="0" step="0.01" /><span class="pointer-events-none absolute right-3.5 text-[15px] text-black/45">{{ __('sell.form.currency') }}</span></div>
            </x-ui.field>
          </div>

          <x-ui.field class="flex-1" :label="__('sell.form.description_label')" for="pDesc">
            <x-ui.textarea :class="$control . ' min-h-24 resize-y leading-[1.5]'" id="pDesc" :placeholder="__('sell.form.description_placeholder')" />
          </x-ui.field>
        </div>
      </div>
