...
<main class="wrap">
  <div class="inner flex flex-col gap-7 pt-11 pb-[72px]">
    ...
    <div class="flex flex-col gap-2">
      ...
      <p class="text-sm text-black/40">{{ t('legal.last_updated', ['date' => $page->updated_at->translatedFormat('j F Y')]) }}</p>
    </div>
    ...
    <div class="flex gap-10 max-[900px]:flex-col">
      ...
      <aside class="w-[300px] shrink-0 max-[900px]:w-full">
        <nav class="sticky top-6 flex flex-col gap-1 rounded border border-black/10 bg-white px-[18px] py-5" aria-label="{{ t('legal.on_this_page') }}">
          <p class="mb-1 text-xs font-semibold uppercase tracking-wide text-black/40">{{ t('legal.on_this_page') }}</p>
      ...
      <article class="legal-content min-w-0 flex-1">
        ...
        <div class="mt-8 rounded bg-[#f5f7f9] px-5 py-[18px]">
          <p class="text-sm font-medium leading-[22px] text-black/50">{{ t('legal.note') }}</p>
        </div>
      </article>
