  ...
  <x-cabinet.card class="bpn-card" tag="h3" gap="gap-0"
    ...
    @foreach (['order' => 'orders', 'reviews' => 'reviews', 'stock' => 'stock', 'report' => 'report'] as $type => $setting)
      <div class="bpn-row">
        ...
        <x-ui.toggle size="md" :data-setting="$setting" :on="(bool) ($settings[$setting] ?? false)" :aria-label="__('business-profile-notifications.types.' . $type . '_title')" />
      </div>
    @endforeach
  </x-cabinet.card>
  ...
  <x-cabinet.card tag="h3" gap="gap-1"
    ...
    <div class="bpn-chips">
      @foreach (['email', 'sms', 'push', 'telegram'] as $channel)
        <x-ui.chip size="md" :data-channel="$channel" :on="in_array($channel, $settings['channels'] ?? [])" :label="__('business-profile-notifications.channels.' . $channel)" />
      @endforeach
    </div>
  </x-cabinet.card>
