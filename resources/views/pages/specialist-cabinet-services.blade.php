...
<x-layout page="specialist-cabinet-services" :title="t('specialist-cabinet-services.title')" bodyClass="bg-gray-soft2">
...
<x-cabinet.shell
  ...
  <x-cabinet.card
      layout="row"
      gap="gap-3.5"
      :title="t('specialist-cabinet-services.list.title', ['count' => count($services)])"
      :desc="t('specialist-cabinet-services.list.summary')"
      data-title-tpl="{{ t('specialist-cabinet-services.list.title', ['count' => ':n']) }}"
      data-new-name="{{ t('specialist-cabinet-services.list.new_name') }}"
      data-new-desc="{{ t('specialist-cabinet-services.list.new_desc') }}">
    <x-slot:action>
    ...
    <div class="scs-list">
      @foreach ($services as $service)
        <x-cabinet.row data-id="{{ $service->id }}">
          ...
          <button type="button" class="scs-grip" data-grip aria-label="{{ t('specialist-cabinet-services.list.reorder') }}"></button>
          ...
          <input type="text" inputmode="decimal" class="scs-price" value="{{ $service->price ? number_format($service->price, 0) : '' }}"
                 aria-label="{{ t('specialist-cabinet-services.list.price_label') }}">

          <select class="scs-unit" aria-label="{{ t('specialist-cabinet-services.list.unit_label') }}">
            @foreach ($units as $unit)
              <option value="{{ $unit }}" @selected(($service->unit ?? 'sqm') === $unit)>{{ t('specialist-cabinet-services.units.' . $unit) }}</option>
            @endforeach
          </select>
          ...
          <x-ui.toggle
              ...
              size="sm"
              tone="ok"
              :aria-label="t('specialist-cabinet-services.list.toggle')" />

          <x-ui.button variant="ghost" class="scs-del" data-del>{{ t('specialist-cabinet-services.list.delete') }}</x-ui.button>
        </x-cabinet.row>
      @endforeach
    </div>
    ...
    <template id="specialistServiceRowTemplate">
      <x-cabinet.row>
        ...
        <select class="scs-unit" aria-label="{{ t('specialist-cabinet-services.list.unit_label') }}">
          @foreach ($units as $unit)
            <option value="{{ $unit }}">{{ t('specialist-cabinet-services.units.' . $unit) }}</option>
          @endforeach
        </select>
    ...
    <p class="scs-hint">{{ t('specialist-cabinet-services.list.hint') }}</p>
  </x-cabinet.card>
